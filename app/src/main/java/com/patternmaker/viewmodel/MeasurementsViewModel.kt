...
class MeasurementsViewModel : ViewModel() {
    ...
    var selectedWaistband   by mutableStateOf(WaistbandType.ELASTIC)
    var enabledOptionalPieces by mutableStateOf(
        TrouserModel.SWEATPANTS.optionalPieces.filter { it.defaultEnabled }.toSet()
    )
    ...
    fun toggleOptionalPiece(piece: OptionalPiece) {
        enabledOptionalPieces = if (enabledOptionalPieces.contains(piece))
            enabledOptionalPieces - piece
        else
            enabledOptionalPieces + piece
    }
    ...
    fun generatePattern() {
        ...
        try {
            ...
            if (m.waist <= 0 || m.hip <= 0 || m.length <= 0) {
                errorMessage = "تأكد من إدخال جميع المقاسات"
                return
            }
            val model = selectedModel.copy(waistbandType = selectedWaistband)
            generatedPieces = PatternEngine.generateTrouser(
                m, model, enabledOptionalPieces.toList()
            )
...
enum class StandardSize(
    val label: String,
    val waist: Int, val hip: Int,
    val length: Int, val legWidth: Int, val rise: Int
) {
    S  ("S",   76,  86,  98, 20, 28),
    M  ("M",   82,  92, 100, 21, 29),
    L  ("L",   88,  98, 102, 22, 30),
